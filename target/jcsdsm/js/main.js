function initmain(listall_url, delete_url, columns, insert_url, str_insert, update_url, str_delete) {
    ...
    $('#dg').datagrid({
        ...
        toolbar: [{//显示工具栏
            ...
            iconCls: 'icon-add',
            handler: function () {
                insertmain(insert_url, str_insert);
            }
        }, '-', {
            text: '更新',
            iconCls: 'icon-edit',
            handler: function () {
                ...
                if (row && row.length > 0) {
                    if (row.length > 1) {
                        ...
                        updatemain(update_url, row, str_delete);
                    }
                } else {
...
function insertmain(insert, str) {
...
function updatemain(update_url, row, str) {
    /*判断页面中是否存在id="dd"元素，若存在则进行删除*/
    if ($("#dd") != null) {
        $("#dd").remove();
    }
